...
function getSchoolListings(school, callback) {
    ...
    request = new Request("SELECT listingID, userTable.userID, listingName, zipCode, prefEmail, grad_year, city, state, lgbtqpFriendly, accessibilityFriendly  FROM listingTable JOIN userTable on (listingTable.userID = userTable.userID) AND org=@School WHERE disabledAcct = 0", function (err, rowCount) {
        if (err) {
            console.error(err);
