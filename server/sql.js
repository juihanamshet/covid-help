...
function getSchoolListings(school, callback) {
    ...
    request = new Request("SELECT listingID, listingName, zipCode, prefEmail, housingRules, grad_year  FROM listingTable JOIN userTable on (listingTable.userID = userTable.userID) AND org='duke' WHERE disabledAcct = 0", function (err, rowCount) {
        if (err) {
            console.log(err);
        }
        else {
